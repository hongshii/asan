...
var footerTop = $('footer').offset().top;   //푸터 위로부터 떨어진 거리
...
//  #conWrap의 높이(세로길이) 구하기 - footer - 아래쪽 fixed사용 안할 떄
var winH = $(window).height();         //현재 열려져 있는 브라우저 높이
var headerH = $('header').height();    // header 높이
var footerH = elFooter.offsetHeight;    // footer 높이 (자바스크립트)
//box-sizing:border-box를 적용시, 패딩, 보더를 제외한 값

console.log(`현재 윈도우의 높이 ${winH} / header의 높이 ${headerH}/ footer의 높이 ${footerH}`);

var conWrapH = winH - (headerH + footerH);  //#conWrap의 높이는 화면 전체의 세로에서 헤더, 푸터 높이를 빼준 값
$('#conWrap').css({height:conWrapH}); 
...
//lnb 슬라이드 메뉴
var du = 400;
var open = 0;       //메뉴가 닫혀 있을 때 (처음)

$('.lnb_btn').click(function on(){
    if(open == 0){
